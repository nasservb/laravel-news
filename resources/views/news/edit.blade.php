                    ...
                    <div class="form-group col-sm-12 col-lg-12">
                        {!! Form::label('content', 'محتوا:') !!}
                         ...
                         <textarea class="form-control" 
                            ...
                            style="margin-top: 0px; margin-bottom: 0px; height: 216px;">{{$news->content}}</textarea>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <select  name="tag[]" class="form-control" multiple id="input-tags" >
                            ...
                            foreach($tags as $id =>$tag )
                            {
                                ...
                                $res .= '<option '. $selected .' value="'. $id .'">'. $tag .'</option>'; 
                            }
                            echo $res;
                        ...
                        {!! Form::label('categories', 'دسته بندی ها:') !!} <br>
                            @php
                                foreach($categories as $key=>$value)
                                {
                                    ...
                                    echo '<input type="checkbox" name="categories[]" '.$checked.'  value="'.$key.'">'.$value.'<br>';
                                }
                            @endphp
                        ...
                        {!! Form::label('status_id', 'انتشار:') !!}
                         <select name="status_id" class="form-control select2 " style="width: 100%;" tabindex="-1" >
                            ...
                            @foreach($statuses as $key=>$value)
                                <option value="{{$key}}" {{$key == $news->status_id ? 'selected' : '' }}>{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
